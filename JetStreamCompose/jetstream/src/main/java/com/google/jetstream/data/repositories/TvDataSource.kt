...
class TvDataSource @Inject constructor(
    ...
    suspend fun getTvShowList(): List<Movie> {
        return try {
            val response = tmdbApiService.getPopularTvShows(apiKey)
    ...
    suspend fun getBingeWatchDramaList(): List<Movie> {
        return try {
            val response = tmdbApiService.getTopRatedTvShows(apiKey)
